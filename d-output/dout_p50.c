...
int p50_write(int fpga_fd, int slot, u8_t *buf, int len)
{
	...
	for(i=0; i<len; i++)
	{
		...
		if(!FpgaWrite(fpga_fd, FPGA_SYS_OUT_PTP_WADDR, i))
		{
			return 0;
		}
		if(!FpgaWrite(fpga_fd, FPGA_SYS_OUT_PTP_WVALUE, buf[i]))
		{
			return 0;
		}
		if(!FpgaWrite(fpga_fd, FPGA_SYS_OUT_PTP_WEN, 0x0000))
		{
			return 0;
		}
		if(!FpgaWrite(fpga_fd, FPGA_SYS_OUT_PTP_WEN, 0x0001))
		{
			return 0;
		}
	}

	if(!FpgaWrite(fpga_fd, FPGA_SYS_OUT_PTP_WREADY, 0x0001))
	{
		return 0;
	}

	if(!FpgaWrite(fpga_fd, FPGA_SYS_OUT_PTP_WREADY, 0x0000))
	{
		return 0;
	}
...
{	
	...
	sprintf(buffer,"%s %02x%02x%02x%02x%02x%02x\n","ipconfig -s",mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
}
